...
export async function registerRoutes(
  ...
  // --- Asset Dashboard ---
  app.get('/api/asset-dashboard', async (req, res) => {
    try {
      // Get all asset-based platforms
      const allPlatforms = await storage.getPlatforms();
      const assetPlatforms = allPlatforms.filter(p => 
        (p as any).platformMode === 'asset_returns' || (p as any).platformMode === 'item_valuations'
      );
      
      // Get assets for each platform
      const platformAssets = await Promise.all(
        assetPlatforms.map(async (platform) => {
      ...
      // Aggregate data
      const allAssets = platformAssets.flatMap(pa => pa.assets);
      const totalInvested = allAssets.reduce((sum, a) => sum + Number(a.investedAmount), 0);
      const totalCurrentValue = allAssets.reduce((sum, a) => sum + (a.currentValue || Number(a.investedAmount)), 0);
      const totalEarnings = allAssets.reduce((sum, a) => sum + (a.profitLoss || 0), 0);
      
      const activeAssets = allAssets.filter(a => a.status === 'active');
      const maturedAssets = allAssets.filter(a => a.status === 'matured');
      const exitedAssets = allAssets.filter(a => a.status === 'exited');
      
      // Group by platform for breakdown
      const platformBreakdown = platformAssets.map(pa => ({
        platformId: pa.platform.id,
        platformName: pa.platform.name,
        platformColor: pa.platform.color,
        currency: (pa.platform as any).currency || 'USD',
        assetCount: pa.assets.length,
        totalInvested: pa.assets.reduce((sum, a) => sum + Number(a.investedAmount), 0),
        totalCurrentValue: pa.assets.reduce((sum, a) => sum + (a.currentValue || Number(a.investedAmount)), 0),
        totalEarnings: pa.assets.reduce((sum, a) => sum + (a.profitLoss || 0), 0),
        activeCount: pa.assets.filter(a => a.status === 'active').length,
        maturedCount: pa.assets.filter(a => a.status === 'matured').length,
      }));
      ...
      for (let i = 11; i >= 0; i--) {
        ...
        for (const asset of allAssets) {
          ...
          if (acquisitionDate <= monthEnd) {
            monthInvested += Number(asset.investedAmount);
            
            // Calculate value at that point in time
            if (asset.annualYield) {
              const yearsElapsed = (monthEnd.getTime() - acquisitionDate.getTime()) / (365 * 24 * 60 * 60 * 1000);
              const accumulatedYield = Number(asset.investedAmount) * (Number(asset.annualYield) / 100) * Math.max(0, yearsElapsed);
              monthValue += Number(asset.investedAmount) + accumulatedYield;
            } else {
              monthValue += Number(asset.investedAmount);
            }
          }
        }
        ...
        monthlyPerformance.push({
          ...
          invested: Math.round(monthInvested * 100) / 100,
        });
      }
      ...
      res.json({
        summary: {
          ...
          exitedCount: exitedAssets.length,
        },
        platformBreakdown,
        monthlyPerformance,
      ...
      res.status(500).json({ message: "Failed to fetch asset dashboard data" });
    }
  });
