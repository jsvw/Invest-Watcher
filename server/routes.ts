...
export async function seedDatabase() {
  ...
  if (platforms.length === 0) {
    ...
    // Create Platforms
    const coinbase = await storage.createPlatform({
      name: "Coinbase",
      category: "Crypto",
    ...
    const robinhood = await storage.createPlatform({
      name: "Robinhood",
      category: "Stocks",
    ...
    const chase = await storage.createPlatform({
      name: "Chase",
      category: "Bank",
    ...
    await storage.createValuation({
      platformId: coinbase.id,
      value: "1800", // Up from 1500
      date: new Date()
    });
    
    await storage.createValuation({
      platformId: robinhood.id,
      value: "2200", // Up from 2000
      date: new Date()
    });
