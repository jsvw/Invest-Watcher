...
- **Supported scrapers**: Monefit SmartSaver (`server/scrapers/monefit.ts`), RoboCash (`server/scrapers/robocash.ts`), CrowdPear (`server/scrapers/crowdpear.ts`), GoldRepublic (`server/scrapers/goldrepublic.ts`), Valvest/Landed.eu (`server/scrapers/valvest.ts`), Trading 212 API (`server/scrapers/trading212.ts`), Stock Ticker (`server/scrapers/stock-ticker.ts`)
- **Stock Ticker Integration**: Uses Yahoo Finance API for live stock prices and frankfurter.app for FX rates. Stores ticker symbol and share count in scraper credentials. Automatically converts stock value from trading currency to platform currency (e.g. USD→EUR). Shows real-time stock position card on platform details page with shares, price, FX rate, and converted value.
