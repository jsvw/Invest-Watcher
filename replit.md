...
- **Single-ticker currency handling**: For single-ticker platforms (e.g. NIO), invested amounts are calculated from actual order history (`/equity/history/orders` endpoint) by summing `walletImpact.netValue` (actual EUR debited per order). Current value is derived as `invested_EUR + ppl + fxPpl` where `ppl` and `fxPpl` are already in the account's primary currency (EUR) from Trading 212. This avoids live FX conversion and provides accurate historical cost basis. The holdings endpoint uses the invested amount from the DB (synced during scrape) for fast responses without re-fetching order history.
...
- **UI**: ScraperConfigDialog component accessible from platform details page ("Web Scraper" button). Shows different credential fields based on scraper type (email/password for Monefit/RoboCash/CrowdPear, username/email/password for GoldRepublic, API key/secret for Trading 212).
