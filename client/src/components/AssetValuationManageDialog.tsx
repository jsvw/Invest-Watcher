...
function AssetValuationHoverContent({ assetId, assetName, currency }: { assetId: number; assetName: string; currency: string }) {
  const currencySymbol = currency === "USD" ? "$" : currency === "EUR" ? "\u20AC" : "";
  const currencySuffix = currency !== "USD" && currency !== "EUR" ? ` ${currency}` : "";
      ...
      <div className="space-y-2">
        ...
        {isLoading ? (
          ...
          <div className="h-32 flex flex-col items-center justify-center">
            <div className="text-lg font-bold">{currencySymbol}{chartData[0].value.toLocaleString()}{currencySuffix}</div>
            <div className="text-xs text-muted-foreground">{format(new Date(chartData[0].date), 'MMM dd, yyyy')}</div>
          </div>
        ) : (
          <div className="h-32">
            <ResponsiveContainer width="100%" height="100%">
              <LineChart data={chartData.map((d: any) => ({ ...d, timestamp: new Date(d.date).getTime() }))} margin={{ top: 5, right: 5, bottom: 5, left: 5 }}>
                ...
                <Line type="monotone" dataKey="value" stroke="#3b82f6" strokeWidth={2.5} dot={{ r: 3, fill: '#3b82f6', strokeWidth: 0 }} activeDot={{ r: 5 }} />
              </LineChart>
            </ResponsiveContainer>
          </div>
        )}
