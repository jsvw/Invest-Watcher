    ...
    return (
      <div className="rounded-xl border border-border bg-card shadow-lg px-4 py-3 text-sm min-w-[200px]">
        ...
        {payload.map((entry: any) => {
          const isPctKey = entry.dataKey === 'gainPct' || entry.dataKey === 'monthlyChangePct';
                ...
                {(chartView === "profit" || chartView === "monthly") && (
                  <div className="flex items-center rounded-md border p-0.5 shrink-0">
                    <button
                      ...
                      className={cn("px-2.5 py-1 text-xs font-medium rounded transition-colors", chartValueMode === "value" ? "bg-primary text-primary-foreground" : "text-muted-foreground hover:text-foreground")}
                      data-testid="chart-toggle-value"
                    >
                      {getCurrencySymbol(currency)}
                    </button>
                    <button
                      type="button"
                      onClick={() => setChartValueMode("pct")}
                      className={cn("px-2.5 py-1 text-xs font-medium rounded transition-colors", chartValueMode === "pct" ? "bg-primary text-primary-foreground" : "text-muted-foreground hover:text-foreground")}
                      data-testid="chart-toggle-pct"
                    >
                      %
                    </button>
                  </div>
                    ...
                    return (
                      <ResponsiveContainer width="100%" height="100%">
                        <BarChart data={mappedData} barCategoryGap="20%">
                          ...
                          {(chartView === "profit" || chartView === "monthly") && (
                            <YAxis yAxisId="left" stroke="hsl(var(--muted-foreground))" fontSize={12} tickLine={false} axisLine={false} tickFormatter={value => chartValueMode === "pct" ? `${value >= 0 ? '+' : ''}${value.toFixed(1)}%` : formatAxisValue(value, true)} domain={['auto', 'auto']} />
                          )}
                          ...
                          {chartView === "profit" && (
                            <Bar dataKey={chartValueMode === "pct" ? "gainPct" : "gain"} name="Profit/Loss" yAxisId="left" isAnimationActive={false} radius={[3, 3, 3, 3]}>
                              {mappedData.map((entry: any, i: number) => <Cell key={i} fill={entry.gain >= 0 ? '#10b981' : '#ef4444'} />)}
                            </Bar>
                          )}
                          {chartView === "monthly" && (
                            <Bar dataKey={chartValueMode === "pct" ? "monthlyChangePct" : "monthlyChange"} name="Monthly Growth" yAxisId="left" isAnimationActive={false} radius={[3, 3, 3, 3]}>
                    ...
                    <ResponsiveContainer width="100%" height="100%">
                      <LineChart data={mappedData}>
                        ...
                        {(chartView === "profit" || chartView === "all") && (
                          <YAxis yAxisId="right" orientation="right" stroke="#10b981" fontSize={12} tickLine={false} axisLine={false} tickFormatter={value => (chartView === "profit" && chartValueMode === "pct") ? `${value >= 0 ? '+' : ''}${value.toFixed(1)}%` : formatAxisValue(value, true)} domain={['auto', 'auto']} />
                        )}
                        {(chartView === "monthly" || chartView === "all") && (
                          <YAxis yAxisId="monthly" orientation="right" stroke="#f59e0b" fontSize={12} tickLine={false} axisLine={false} tickFormatter={value => (chartView === "monthly" && chartValueMode === "pct") ? `${value >= 0 ? '+' : ''}${value.toFixed(1)}%` : formatAxisValue(value, true)} domain={['auto', 'auto']} />
                        )}
                        <Tooltip content={<ChartTooltip />} />
                        <Legend verticalAlign="top" height={36} />
                        {(chartView === "overview" || chartView === "all") && (
                        ...
                        {(chartView === "profit" || chartView === "all") && (
                          <Line type="monotone" dataKey={chartView === "profit" && chartValueMode === "pct" ? "gainPct" : "gain"} name="Profit/Loss" yAxisId="right" stroke="#10b981" strokeWidth={2} dot={false} activeDot={{ r: 4 }} isAnimationActive={false} />
                        )}
                        {(chartView === "monthly" || chartView === "all") && (
                          <Line type="monotone" dataKey={chartView === "monthly" && chartValueMode === "pct" ? "monthlyChangePct" : "monthlyChange"} name="Monthly Growth" yAxisId="monthly" stroke="#f59e0b" strokeWidth={2} dot={{ r: 5, fill: '#f59e0b', strokeWidth: 0 }} activeDot={{ r: 7 }} isAnimationActive={false} />
                        )}
                      </LineChart>
                    </ResponsiveContainer>
                  );
